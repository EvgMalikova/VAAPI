...
void RenderWindowInteractor::guiSoundRenderingProc()
{
    ...
    for (int i = 0; i < m_window->GetRenderer()->GetNumOfActors(); i++) {
        optixBasicActor* a = m_window->GetRenderer()->GetActor(i);
        //a->PrintInfo();  - works
        a->SetAuditoryModel();
    }
...
void BasicRenderWindowInteractor::guiWidgetProc()
{
    ...
    //Update widget
   /* if (m_widget != nullptr) {
        if (m_widget->updateCam) {
            optix::float3 pos;
            optix::float3 v, u, w;
            m_window->m_pinholeCamera->getFrustum(pos, u, v, w);
            // std::cout << u.x <<","<< v.y << std::endl;
            m_widget->SetCamParameters(pos, u, v, w);
        }
    }*/
