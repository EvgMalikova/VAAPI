...
rtDeclareVariable(PerAudioRayData, prd2, rtPayload, );
...
inline __device__ float3 transfer_function(int t)
{
    ...
    case 6: //P
        ...
        break;
    case 7: //Si
        return make_float3(226 / 255, 203 / 255, 182 / 255);
...
RT_PROGRAM void auditory_volume_any_hit()
{
    ...
    if (prd2.cur_prim < MAX_PRIM_ALONG_RAY)
    {
        prd2.primitives[prd.cur_prim] = make_float2(info.primInfo.x, float(info.type));// info.primInfo;
        prd2.cur_prim++;
...
RT_PROGRAM void auditory_raytrace_hit()
{
    ...
    if (prd2.cur_prim < MAX_PRIM_ALONG_RAY)
    {
        prd2.primitives[prd.cur_prim] = make_float2(info.primInfo.x, 5.0);//float(info.type));// info.primInfo;
        prd2.cur_prim++;
